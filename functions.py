...
def report_b_request(msg, lang):
    ...
    try:
        ...
        max_points = passed_hours * MAX_GPA
        mj = ''
        if points > max_points or semester > NUMBER_OF_SEMESTERS or passed_hours > TOTAL_HOURS[mj] or passed_hours > registered_hours or points < passed_hours:
            return -2
    except Exception as e:
        print(e)
        return -1
...
def get_report(type, passed_hours, points, semester, lang, registered_hours, mj):
    ...
    hours_percentage = get_hours_percentage(passed_hours)
    rank_estimation = get_rank_estimation(gpa)
    on_plan = is_on_plan(semester, passed_hours, lang)
    remaining_hours = get_remaining_hours(passed_hours)
...
def error_log(e):
    ...
    with open('errorsLog.txt', 'a') as file:
        file.write(dt.strftime("%A %d %B %Y %H:%M") +'\t\t' + str(e) + '\n\n')
